...
let rec transProg (e: Ab.exp) : (transty * En.err list) =
  ...
  let ty = transExp En.base_venv En.base_tenv e new_level false
  in (ty, !semErrors)
...
and transExp (venv: En.fnvar Sy.table) (tenv: En.ty Sy.table)
  ...
  let rec tExp = function
    ...
    | Ab.LetExp{decs; body; pos} ->
       ...
       let (venv', tenv') = transDecs venv tenv decs lev in
       transExp venv' tenv' body lev false
    ...
    | Ab.WhileExp {test; body; pos} ->
      ...
      if not (checkTyHelper (transExp venv tenv body lev true) En.UNIT) then
        report pos (En.UnexpectedType (En.UNIT, (transExp venv tenv body lev true))) "The body expression of while expression need to be unit.";
      En.UNIT
    ...
    | Ab.ForExp {var; escape; lo; hi; body; pos} ->
          ...
          let venv' =
            Sy.enter venv var (En.VarEntry{access = Tr.alloc_local lev !escape; ty = En.INT; loopvar = true})
            in
            if not (checkTyHelper (transExp venv' tenv body lev true) En.UNIT) then 
             report pos (En.UnexpectedType (En.UNIT, (transExp venv' tenv body lev true))) "The body expression of while expression need to be unit.";
            En.UNIT
   ...
   let (venvPlusF, localVenv') = 
     ...
     | Some (En.FunEntry {level; label; formals ; result})
            -> report fnpos (En.Duplicates (Sy.name fnname)) "Already have a formal in this symbol.";
               (Sy.enter venv fnname (En.FunEntry {level = (Tr.new_level {parent = lev; name = tempLabel; formals = esc}) ;label = tempLabel; formals = (List.map snd fparam); result = rType}), 
                Sy.enter localVenv fnname (En.FunEntry {level = (Tr.new_level {parent = lev; name = tempLabel; formals = esc}) ;label = tempLabel; formals = (List.map snd fparam); result = rType}))
     | _ ->
           (Sy.enter venv fnname (En.FunEntry {level = (Tr.new_level {parent = lev; name = tempLabel; formals = esc}) ;label = tempLabel; formals = (List.map snd fparam); result = rType}), 
           Sy.enter localVenv fnname (En.FunEntry {level = (Tr.new_level {parent = lev; name = tempLabel; formals = esc}) ;label = tempLabel; formals = (List.map snd fparam); result = rType}))
     in
  ...
  and checkFuncVar (venv: En.fnvar Sy.table) (tenv:En.ty Sy.table) ({Ab.fnname; params; result; body; fnpos}) (lev: Tr.level)= 
  let rType = match result with
     | None -> En.UNIT
     | Some(sy, pos) -> 
     match Sy.look tenv sy with
  ...
  let fparam2 = 
   ...
   let bodyT =
     transExp venvPlusP tenv body curLevel false
   in
  ...
  and transDec (venv: En.fnvar Sy.table) (tenv: En.ty Sy.table)
               (d: Ab.dec) (lev: Tr.level)
      : (En.fnvar Sy.table * En.ty Sy.table )
    = begin match d with
    | Ab.VarDec{name; escape; typ; init; pos} -> 
      let tInit = transExp venv tenv init lev false in
      let tVar = 
        ...
        | None -> 
          ...
          in
        Sy.enter venv name (En.VarEntry{access = Tr.alloc_local lev !escape; ty = tVar; loopvar = false}), tenv
   | Ab.TypeDec types ->
   ...
   | Ab.FunctionDec fundecs ->
     ...
     in 
     List.iter (fun dec -> checkFuncVar venvWithFunHead tenv dec lev) fundecs;
  ...
  and transDecs (venv: En.fnvar Sy.table) (tenv: En.ty Sy.table) 
                (ds: Ab.dec list) (lev: Tr.level)
      : (En.fnvar Sy.table * En.ty Sy.table)
    = 
      List.fold_left
      (fun (venv, tenv) dec ->transDec venv tenv dec lev) (venv, tenv) ds
