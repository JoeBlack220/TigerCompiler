...
module Translate 
  ...
  let ifThenElseExp testExp thenExp elseExp = 
    ...
    let conds() = Cx(fun (t', f') -> seq[ mkCx testExp (t, f);
                                          ...
                                          mkCx elseExp (t', f')])
    in 
    let nores() = Nx(seq [ mkCx testExp (t, f);
                           Ir.LABEL t;
                           mkNx thenExp;
...
let seqExp (exps: exp list) = 
  ...
  let simpleVar (acce: access) curlev =
    let (acc, frameacc) = acce in
    let (usefram, uselevel, useuniq) =
  ...
  let procEntryExit lev bexp = 
    ...
    let treeBody = (mkEx bexp) in
      (fragments := (Fr.proc_frag levelFrame (Fr.proc_body levelFrame treeBody)) :: (!fragments))
  ...
  let main (lev: level) (e: exp) : unit = match lev with
    | Other (fr, _, _) ->
       let new_frag = Fr.proc_frag fr (Fr.proc_body fr (mkEx e)) in
