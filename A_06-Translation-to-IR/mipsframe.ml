...
module MipsFrame (Tm: Temp.TEMP)
  ...
  type frag = PROC of {body: Ir.stm; frame:frame} 
            | STRING of label * string

  let string_frag lab str = STRING (lab,str)

  let proc_frag fr stm = PROC {body = stm; frame = fr}
  ...
  let proc_body ({epilogue=lab}:frame) exp = 
    Ir.SEQ (Ir.MOVE (ret_val, exp), Ir.LABEL lab)
  ...
  let rec getExp (framelist: frame list) =
    ...
    | [] ->
      (Ir.TEMP fp_reg)
    | fr :: rest -> 
      begin match (List.hd (formals fr)) with
      | InReg temp -> Ir.TEMP temp
      | InFrame k ->
      (Ir.MEM (Ir.BINOP (Ir.PLUS, Ir.CONST k, (getExp rest))))
      end
    end
