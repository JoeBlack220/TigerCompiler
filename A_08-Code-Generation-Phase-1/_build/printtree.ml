...
module type PRINTTREE = sig
  ...
  val print_tree: out_channel -> stm list -> string -> unit 
...
module PrintTree (Sy: Symbol.SYMBOL)
  ...
  let print_tree out_chan s0 tempPrefix =
    ...
    and stm_lst stms =
      let rec stm_lst_aux = function
        | [s] -> (stm s 0; sayln " "; sayln "]")
        | s::sl -> (stm s 0; sayln ","; stm_lst_aux sl)
        | _ -> say ""
      in
      match stms with
      | [] -> say "[]"
      | _ -> sayln "["; stm_lst_aux stms
  ...
  let normalize frags = 
      ...
      let lNewLabel k
        = if String.sub (Sy.name k) 0 1 = "$" 
          then k
               ...
               | None -> failwith ("Error1: Tiger Compiler Bug: in lLook")
      ...
      let tUpdate k = match Sy.look (!tTable) k with
        ...
        | None -> let v = !tNew in
                  ...
                  tTable := new_table
      in
      let tNewTemp t =  match Sy.look (!tTable) t with
        | None -> failwith ("Error2: Tiger Compiler Bug: in lLook")
        | Some n -> Tm.named_temp n
      in
      let rec findStms = function
        | [] -> ()
        | s::ss -> findStm s; findStms ss
      and findStm = function
      ...
      let rec findFrags = function
        ...
        | frag::rest ->
           ...
           findFrags rest
      in
      let rec fixStms = function
        | [] -> []
        | s::ss -> fixStm s :: fixStms ss
      and fixStm = function
