...
%token <int * Analysis.Ab.pos> INT
%token <string * Analysis.Ab.pos> STRING
%token <string * Analysis.Ab.pos> ID
%token <Analysis.Ab.pos> FOR 
%token <Analysis.Ab.pos> WHILE 
%token <Analysis.Ab.pos> BREAK 
%token <Analysis.Ab.pos> LET 
%token <Analysis.Ab.pos> IN 
%token <Analysis.Ab.pos> NIL 
%token <Analysis.Ab.pos> TO 
%token <Analysis.Ab.pos> END
%token <Analysis.Ab.pos> FUNCTION 
%token <Analysis.Ab.pos> VAR 
%token <Analysis.Ab.pos> TYPE 
%token <Analysis.Ab.pos> ARRAY 
%token <Analysis.Ab.pos> IF 
%token <Analysis.Ab.pos> THEN 
%token <Analysis.Ab.pos> ELSE 
%token <Analysis.Ab.pos> DO 
%token <Analysis.Ab.pos> OF
%token <Analysis.Ab.pos> LPAREN
%token <Analysis.Ab.pos> RPAREN 
%token <Analysis.Ab.pos> LBRACK 
%token <Analysis.Ab.pos> RBRACK 
%token <Analysis.Ab.pos> LBRACE 
%token <Analysis.Ab.pos> RBRACE
%token <Analysis.Ab.pos> DOT 
%token <Analysis.Ab.pos> COLON 
%token <Analysis.Ab.pos> COMMA 
%token <Analysis.Ab.pos> SEMICOLON
%token <Analysis.Ab.pos> PLUS 
%token <Analysis.Ab.pos> MINUS 
%token <Analysis.Ab.pos> TIMES
%token <Analysis.Ab.pos> DIVIDE
%token <Analysis.Ab.pos> EQ 
%token <Analysis.Ab.pos> NEQ 
%token <Analysis.Ab.pos> LT 
%token <Analysis.Ab.pos> LE 
%token <Analysis.Ab.pos> GT 
%token <Analysis.Ab.pos> GE
%token <Analysis.Ab.pos> AND 
%token <Analysis.Ab.pos> OR
%token <Analysis.Ab.pos> ASSIGN
%token <Analysis.Ab.pos> UMINUS
%token EOF
...
| exp PLUS exp { A.OpExp {left = $1; oper = A.PlusOp; right = $3; pos = $2 } }
...
| exp EQ exp { A.OpExp {left = $1; oper = A.EqOp; right = $3; pos = $2 } }
...
| FOR id ASSIGN exp TO exp DO exp { A.ForExp{ var = $2; escape = ref false; lo = $4; hi = $6; body = $8;pos = $1 } }
| BREAK { A.BreakExp ($1)}
...
field:
  id COLON id { {fname = $1; escape = ref false; typ = $3; fpos = Parsing.rhs_start 1} }
...
vardec:
  VAR id ASSIGN exp { A.VarDec{name = $2; escape = ref false; typ = None; init = $4; pos = $1} }
| VAR id COLON id ASSIGN exp { A.VarDec{name = $2; escape = ref false; typ = Some($4,Parsing.rhs_start 4); init = $6; pos = $1} }
